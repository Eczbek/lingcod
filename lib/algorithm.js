...
export function mergeSort(array, sortCallback = (value1, value2) => value1 - value2) {
    if (array.length < 2)
        return array;
...
export function heapSort([...array], sortCallback = (value1, value2) => value1 - value2) {
    function heapify(chunk, size, index) {
        let min = index;
        const left = index * 2 + 1;
        const right = left + 1;
        if (left < size && sortCallback(chunk[left], chunk[min]) > 0)
            min = left;
        if (right < size && sortCallback(chunk[right], chunk[min]) > 0)
            min = right;
        if (min !== index) {
