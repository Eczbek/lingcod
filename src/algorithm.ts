...
export function binarySearch(sortedArray: any[], searchCallback: (value: any, index: number) => number): number {
	let left = 0;
...
export function mergeSort(array: any[], sortCallback = (value1: any, value2: any) => value1 - value2): any[] {
	...
	while (left.length && right.length)
		sorted.push((sortCallback(left[0], right[0]) > 0 ? right : left).shift());
	return [...sorted, ...left, ...right];
}

export function heapSort([...array]: any[], sortCallback = (value1: any, value2: any) => value1 - value2): any[] {
	function heapify(chunk: any[], size: number, index: number) {
		let min = index;
		const left = index * 2 + 1;
		const right = left + 1;
		if (left < size && sortCallback(chunk[min], chunk[left]) < 0)
			min = left;
		if (right < size && sortCallback(chunk[min], chunk[right]) < 0)
			min = right;
		if (min !== index) {
