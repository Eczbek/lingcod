...
export function range (max: number, min = 0, step = 1) {
	const array = [];
	for (let i = min; i < max; i += step) array.push(i);
	return array;
}

export function fill (length: number, fillCallback: (index: number) => any) {
	const array = [];
	for (let i = 0; i < length; ++i) array.push(fillCallback(i));
	return array;
}

export function includesAll (array: any[], values: any[]) {
	return values.every((value) => array.includes(value));
}

export function filterByOccurences (array: any[], occurences: number, compareCallback = (value1: any, value2: any) => value1 === value2) {
...
export function chunk ([...array], getChunkSize: (value: any, index: number) => number, overflow = false) {
	const chunked = [];
	while (true) {
		const size = getChunkSize(array[chunked.length], chunked.length);
		if (array.length < size && !overflow || !array.length) return chunked;
		chunked.push(array.splice(0, size));
	}
}

export function filterIndices (array: any[], filterCallback: (value: any, index: number) => boolean) {
	const indices = [];
	for (let i = 0; i < array.length; ++i) if (filterCallback(array[i], i)) indices.push(i);
	return indices;
}
...
export function findEmptyIndices (array: any[]) {
	return filterIndices(array, (_, index) => !Object.hasOwn(array, index));
}

export function isSparse (array: any[]) {
